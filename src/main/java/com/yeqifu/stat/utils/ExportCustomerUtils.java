...
public class ExportCustomerUtils {
    ...
    public static ByteArrayOutputStream exportCustomer(List<Customer> customers, String sheetName) {
        ...
        //1.8.1第三行
        String[] titles={"身份证号","客户姓名","客户电话","客户地址","客户职业","审核状态","录入时间"};
        index++;
        ...
        for (int i = 0; i < customers.size(); i++) {
            ...
            //1.9.1创建列客户性别
            HSSFCell row_isexamine = row.createCell(5);
            row_isexamine.setCellStyle(baseStyle);
            if(customer.getIsexamine() == 0){
                row_isexamine.setCellValue("未审核");
            }else if(customer.getIsexamine() == 1){
                row_isexamine.setCellValue("审核通过");
            }else if(customer.getIsexamine() == 2){
                row_isexamine.setCellValue("审核不通过");
            }
            //1.9.1创建列客户录入时间
